...
- The test suite no longer depends on file order through global Settings,
  ...
  bibr's default ports held. Tests that intentionally reach the network
  (live API and Hub downloads) are marked `network` and opt out of the
  socket guard. Core CI now runs
