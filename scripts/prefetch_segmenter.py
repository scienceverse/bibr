...
def _prefetch_remote_to_cache(repo_id: str, cache_dir: Path) -> tuple[str, str]:
    ...
    onnx_path = Path(hf_hub_download(repo_id, "model_optimized.onnx", **download_kwargs))
    config_path = Path(hf_hub_download(repo_id, "config.json", **download_kwargs))
    if config_path.parent != onnx_path.parent:
        raise RuntimeError("Prefetched wtpsplit model files resolved to different snapshots")

    try:
        tokenizer_path = Path(hf_hub_download(repo_id, "tokenizer.json", **download_kwargs))
...
def prefetch_segmenter(model: str, *, cache_dir: Path | None) -> None:
    """Instantiate a CPU ORT segmenter so its model assets are cached."""
    from wtpsplit_lite import SaT

    resolved = resolve_wtpsplit_model(model)
...
def main() -> None:
    ...
    prefetch_segmenter(args.model, cache_dir=args.cache_dir)
